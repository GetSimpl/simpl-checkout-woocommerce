...
class SimplCheckoutCouponController {
    function scwp_apply(WP_REST_Request $request)
    {
        try {
            ...
            $cart = SimplWcCartHelper::scwp_load_cart_from_order($order_id);
    ...
    function scwp_remove(WP_REST_Request $request)
    {
        try {
            global $notice_message;
            simpl_cart_init_common();
            ...
            $cart = SimplWcCartHelper::scwp_load_cart_from_order($order->get_id());
            ...
            $order->save();
            $si = new SimplCartResponse();
            return $si->scwp_cart_payload(WC()->cart, $order->get_id());
        } catch (SimplCustomHttpBadRequest $fe) {
            return new WP_REST_Response(array("code" => SIMPL_HTTP_ERROR_BAD_REQUEST, "message" => $fe->getMessage()), 400);
        } catch (Exception $fe) {
	        return new WP_REST_Response(array("code" => SIMPL_HTTP_ERROR_USER_NOTICE, "message" => $fe->getMessage()), 500);
    ...
    function scwp_remove_all(WP_REST_Request $request)
    {
        try {
            global $notice_message;
            simpl_cart_init_common();
