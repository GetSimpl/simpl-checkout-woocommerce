...
class SimplCheckoutOrderController
{
    function scwp_fetch(WP_REST_Request $request)
    {
        try {
            ...
            $order_payload = $si->scwp_order_payload($order);
    ...
    function scwp_create(WP_REST_Request $request)
    {
        try {
            ...
            $gateway = $this->scwp_simpl_gateway($order->get_id());
            if (!$gateway) {
    ...
    protected function scwp_update_order_metadata($request, $order)
    {
    ...
    protected function scwp_reset_session()
    {
        WC()->session->set("simpl_order_id", null);
        WC()->session->set("simpl:session:id", null);
    }

    protected function scwp_simpl_gateway($order_id)
    {
